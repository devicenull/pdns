 ...
 *
 * In addition, for the avoidance of any doubt, permission is granted
 * to link this program with OpenSSL and to (re)distribute the binaries
...
BOOST_AUTO_TEST_SUITE(test_auth_xsk)
...
BOOST_AUTO_TEST_CASE(test_XskSocket)
{
  ...
  {
    /* the XSK map is not pinned at this path, should throw */
...
{
  const MACAddr fromMAC{};
  const MACAddr toMAC{0x01, 0x02, 0x03, 0x04, 0x05, 0x06};
